...
class Create {
  constructor(options) {
    ...
    for (var i = 0; i < tap.length; i++) {
      if (fs.existsSync(path.join(process.cwd(), `${this.path}/${tap[i]}`)) === false) {
        fs.mkdirSync(path.join(process.cwd(), `${this.path}/${tap[i]}`));
      }
      if (fs.existsSync(path.join(process.cwd(), this.path, tap[i], "storage.json")) === false) {
          fs.writeFileSync(path.join(process.cwd(), this.path, tap[i], "storage.json"), "{}");
      }
  ...
  deleteAll(table) {
    ...
    // Эта функция удалит всё содержимое table
  }
  ...
  ping() {
    ...
    /*
     * Эта функция выдаёт задержку
     * ping - ms
    */
  }
  
  editName(table, key, newkey) {
    let db = get(table, this.path);
    db[newkey] = db[key];
    delete db[key];
    set(table, this.path, db);
  ...
  /*
   * Эта функция меняет название переменной на новое, без потерь содержимого
   * key - название переменной
   * newkey - новое название переменной
  */
  }
