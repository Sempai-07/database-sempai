...
export class CreateSqlite extends EventEmitter {
    constructor(options) {
        super();
        ...
        for (var i = 0; i < table_file.length; i++) {
            ...
            if (fs.existsSync(path.join(process.cwd(), this.path, table_file[i], "storage.sqlite")) === false) {
                fs.writeFileSync(path.join(process.cwd(), this.path, table_file[i], "storage.sqlite"), "{}");
            }
        }
    }
    set(table, key, value, encryption = false) {
        ...
        let db = get("storage.sqlite", table, this.path);
        let values = value;
        if (encryption) {
            if (!this.key)
        ...
        set("storage.sqlite", table, this.path, db);
    }
    ...
    delete(table, key, oldValue = false) {
        if (!this.table.find(a => a === table))
            throw new DatabaseError("Invalid table name: " + table);
        let db = get("storage.sqlite", table, this.path);
        let values = db[key]['value'];
    ...
    deleteAll(table) {
        if (!this.table.find(a => a === table))
            throw new DatabaseError("Invalid table name: " + table);
        fs.writeFileSync(path.join(process.cwd(), this.path, table, "storage.sqlite"), "{}");
    }
    has(table, key) {
        if (!this.table.find(a => a === table))
            throw new DatabaseError("Invalid table name: " + table);
...
//# sourceMappingURL=CreateSqlite.js.map