...
export class CreateSqlite extends EventEmitter {
    constructor(options) {
        ...
        for (var i = 0; i < table_file.length; i++) {
            ...
            if (fs.existsSync(path.join(process.cwd(), this.path, table_file[i], "storage.sqlie")) === false) {
                fs.writeFileSync(path.join(process.cwd(), this.path, table_file[i], "storage.sqlie"), "{}");
            }
        }
    }
    set(table, key, value, encryption = false) {
        ...
        let db = get("storage.sqlie", table, this.path);
        let values = value;
        if (encryption) {
            if (!this.key)
        ...
        set("storage.sqlie", table, this.path, db);
    }
    ...
    deleteAll(table) {
        if (!this.table.find(a => a === table))
            throw new DatabaseError("Invalid table name: " + table);
        fs.writeFileSync(path.join(process.cwd(), this.path, table, "storage.sqlie"), "{}");
    }
    has(table, key) {
        if (!this.table.find(a => a === table))
            throw new DatabaseError("Invalid table name: " + table);
